...
class DefaultController extends Controller
{
	...
	public function accessRules()
	{
        ...
        {
             $arr =array('admin','update','delete','view', 'index');
        }
        else if( Yii::app()->user->getState('role') == "2")
        {
            $arr =array('update', 'view', 'index', 'admin'); 
        }
        else if( Yii::app()->user->getState('role') == "3")
        {
          	$arr = array('index', 'update', 'view', 'bimbinganD', 'listAktif', 'listKosong', 'listCuti', 'listLulus', 'ajaxStatusKRSSave'); 
        }
        else
        {
        	$arr = array('');
	...
	public function actionBimbinganD()
	{
		...
		$this->render('bimbingan_d',array(
			'model'=>$model,
		));
	}	

	public function actionListAktif()
	{
		$model=new Mahasiswa('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Mahasiswa']))
		...
		$this->render('listAktif',array(
			'model'=>$model,
		));
	}	

	public function actionListKosong()
	{
		$model=new Mahasiswa('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Mahasiswa']))
	...
	public function actionListCuti()
	{
		...
		$this->render('listCuti',array(
			'model'=>$model,
		));
	}	

	public function actionListLulus()
	{
		$model=new Mahasiswa('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Mahasiswa']))
		...
		$this->render('listLulus',array(
			'model'=>$model,
		));
	}

	public function actionAjaxStatusKRSSave()
	{
		$setAll = $_POST['status_krs'];
		if(count($setAll) > 0)
		{
			foreach($setAll as $id_mhs => $status_krs)
			{
