...
class tlbExcelView extends CGridView
{
    ...
    //buttons config
    public $exportButtonsCSS = 'btn btn-default';
    ...
    public function renderExportButtons()
    {
        foreach ($this->exportButtons as $key => $button) {
            ...
            if (key_exists('query', $url)) {
                $content[] = CHtml::link($item['caption'], '?' . $url['query'] . '&exportType=' . $type . '&' . $this->grid_mode_var . '=export', array('class' => 'btn btn-default'));          
            } else {
                $content[] = CHtml::link($item['caption'], '?exportType=' . $type . '&' . $this->grid_mode_var . '=export', array('class' => 'btn btn-default'));				
            }
        }
        if ($content) {
            echo CHtml::tag('div', array(), implode(', ', $content));	
        }
